...
interface KeyProps {
  ...
  isActive?: boolean;   // ¿Está siendo tocada por el teclado de la PC?
  onPress: (freq: number) => void;  // Función que avisa "me están apretando"
  onRelease: (freq: number) => void;            // Función que avisa "me soltaron"
}

export const Key = ({ note, frequency, isBlack = false, isActive = false, onPress, onRelease }: KeyProps) => { 

  const { isHovered, hoverHandlers } = useHover();
  const [isPressed, setIsPressed] = useState(false);

  // Clase dinámica: Si isBlack es true, añade la clase "black"
  const className = `key-tooth ${isBlack ? 'black' : ''} ${isPressed || isActive ? 'pressed' : ''}`;

  const handleMouseEnter = (e: React.MouseEvent) => {
    // Mostrar el tooltip 
    hoverHandlers.onMouseEnter();
    // Si el usuario está arrastrando el mouse con el botón izquierdo apretado, también queremos tocar la nota
    if (e.buttons === 1) {
      onPress(frequency); 
      setIsPressed(true);
    }
  ...
  // Pequeño ajuste para evitar que se seleccione el texto al arrastrar
